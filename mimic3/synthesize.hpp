...
namespace mimic3
{
    ...
    mimic3_Session createSession(std::filesystem::path modelPath)
    {
        ...
        {
            auto tmp = session.onnx.GetInputNameAllocated(i, session.allocator);
            spdlog::info("GetInputNameAllocated: input name {}", tmp.get());
            session.inputNames.push_back(tmp.get());
            session.inputNamesStore.push_back(std::move(tmp));
        }

        for (size_t i = 0; i < numOutputNodes; i++)
        {
            auto tmp = session.onnx.GetOutputNameAllocated(i, session.allocator);
            spdlog::info("GetOutputNameAllocated: {}", tmp.get());
            session.outputNames.push_back(tmp.get());
            session.outputNamesStore.push_back(std::move(tmp));
        }
